 ...
 * Return: bytes read
 */
ssize_t ft_input_buf(info_t *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
...
ssize_t ft_get_input(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		*buf_p = p; /* pass back pointer to current command position */
		return (ft_strlen(p)); /* return length of current command */
	}
 ...
 * Return: r
 */
ssize_t ft_read_buf(info_t *info, char *buf, size_t *i)
{
	ssize_t r = 0;
