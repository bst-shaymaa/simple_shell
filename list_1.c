...
/**
 * ft_list_to_strings - returns an array of strings of the list->str
...
/**
 * ft_node_starts_with - returns node whose string starts with prefix
...
list_t *ft_node_starts_with(list_t *node, char *prefix, char c)
{
	...
	while (node)
	{
		p = ft_starts_with(node->str, prefix);
		if (p && ((c == -1) || (*p == c)))
			return (node);
		node = node->next;
	}
